...
export const Inputs = ({type, txt, row='4', cols='50', name, placeholder='', styleName, valueSet, optionSect, func}) => {
  switch (type) {
    case 'input':
        return(
    ...
    case 'textarea':
        ...
        case 'select':
            return(
                <article className={`contenF ${styleName}`} data-txt={txt}>
                <select value={valueSet} onChange={func}>
                    <option value={""}>Seleccione</option>
                    {optionSect.map((option, index)=>{
                        return(
                            <option key={index+option} value={option.id}>{option[name]}</option>
                        );
                    })}
                </select> 
            </article>
